...
// This is used to avoid a massive footgun for users of slinky: when defining bounds expresultsions, crops, and so on, it
// is very tempting to use buffer metadata. Doing this is problematic because the buffer metadata will be affected by
// crops and slices generated by slinky, which change the meaning of the expresultsions. This avoids this problem by
// replacing buffer metadata expresultsions with variables, and remembers the values to define these replacements outside
...
public:
  ...
  void visit(const variable* op) override {
    ...
    // Don't lift internally allocated buffer metadata expresultsions.
...
public:
  ...
  stmt build(const stmt& body, const func* base_f, const loop_id& at) {
    ...
    for (const auto& b : candidates_for_allocation_) {
      ...
      if ((b->store_at() && *b->store_at() == at) || (!b->store_at() && at.root())) {
        lifetimes.push_back(
            std::make_tuple(b, allocation_lifetime_start_[b->sym()], allocation_lifetime_end_[b->sym()]));
      }
    }
