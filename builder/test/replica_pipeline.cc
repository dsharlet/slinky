...
auto p = []() -> ::slinky::pipeline {
  using std::abs, std::min, std::max;
  node_context ctx;
  auto in0 = buffer_expr::make(ctx, "in0", /*rank=*/1, /*elem_size=*/1);
  in0->dim(0).bounds.min = 0;
  in0->dim(0).fold_factor = (index_t) 9223372036854775807;
  auto out2 = buffer_expr::make(ctx, "out2", /*rank=*/1, /*elem_size=*/1);
  out2->dim(0).bounds.min = 0;
  out2->dim(0).fold_factor = (index_t) 9223372036854775807;
  auto d0 = var(ctx, "d0");
  auto v1 = buffer_expr::make(ctx, "v1", /*rank=*/1, /*elem_size=*/1);
  v1->dim(0).fold_factor = (index_t) 9223372036854775807;
  auto _replica_fn_2 = [=](const buffer<const void>& i0, const buffer<void>& o0) -> index_t {
    const buffer<const void>* input_buffers[] = {&i0};
    const buffer<void>* output_buffers[] = {&o0};
    const func::input inputs[] = {{in0, {point(d0)}}};
    const std::vector<var> outputs[] = {{d0}};
    return ::slinky::internal::replica_pipeline_handler(input_buffers, output_buffers, inputs, outputs);
  };
  auto _fn_1 = func::make(std::move(_replica_fn_2), {{in0, {point(d0)}}}, {{v1, {d0}}}, {.allow_in_place = 1, .name = "Abs (NC)"});
  auto _replica_fn_3 = [=](const buffer<const void>& i0, const buffer<void>& o0) -> index_t {
    const buffer<const void>* input_buffers[] = {&i0};
    const buffer<void>* output_buffers[] = {&o0};
    const func::input inputs[] = {{v1, {point(d0)}}};
    const std::vector<var> outputs[] = {{d0}};
    return ::slinky::internal::replica_pipeline_handler(input_buffers, output_buffers, inputs, outputs);
  };
  auto _fn_0 = func::make(std::move(_replica_fn_3), {{v1, {point(d0)}}}, {{out2, {d0}}}, {.allow_in_place = 1, .name = "Negate (NC)"});
  auto _4 = in0->sym();
  auto _5 = min((buffer_max(_4, 0)), 131071);
  auto _6 = max(_5, 0);
  auto _7 = (_6 + 1);
  _fn_0.loops({{d0, _7, loop::serial}});
  _fn_0.compute_root();
  auto out3 = buffer_expr::make(ctx, "out3", /*rank=*/3, /*elem_size=*/4);
  out3->dim(0).bounds.min = 0;
  out3->dim(0).fold_factor = (index_t) 9223372036854775807;
  out3->dim(1).bounds.min = 0;
  out3->dim(1).fold_factor = (index_t) 9223372036854775807;
  out3->dim(2).bounds.min = 0;
  out3->dim(2).fold_factor = (index_t) 9223372036854775807;
  auto p = build_pipeline(ctx, {}, {in0}, {out2, out3}, {}, {});
  return p;
};
// END define_replica_pipeline() output
  // clang-format on
