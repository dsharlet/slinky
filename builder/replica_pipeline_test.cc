...
auto p = []() -> ::slinky::pipeline {
  ...
  node_context ctx;
  auto a = buffer_expr::make(ctx, "a", 2, 4);
  a->dim(1).stride = 4;
  auto b = buffer_expr::make(ctx, "b", 2, 4);
  b->dim(1).stride = 4;
  auto c = buffer_expr::make(ctx, "c", 2, 4);
  c->dim(1).stride = 4;
  auto abc = buffer_expr::make(ctx, "abc", 2, 4);
  abc->dim(1).stride = 4;
  auto i = var(ctx, "i");
  auto j = var(ctx, "j");
  auto ab = buffer_expr::make(ctx, "ab", 2, 4);
...
auto p = []() -> ::slinky::pipeline {
  using std::abs, std::min, std::max;
  node_context ctx;
  auto in = buffer_expr::make(ctx, "in", 2, 4);
  auto out = buffer_expr::make(ctx, "out", 2, 4);
  auto x = var(ctx, "x");
  auto y = var(ctx, "y");
  auto intm = buffer_expr::make(ctx, "intm", 2, 4);
...
auto p = []() -> ::slinky::pipeline {
  using std::abs, std::min, std::max;
  node_context ctx;
  auto in = buffer_expr::make(ctx, "in", 3, 4);
  auto sum_x = buffer_expr::make(ctx, "sum_x", 2, 4);
  auto y = var(ctx, "y");
  auto z = var(ctx, "z");
  ...
  auto sum_xy = buffer_expr::make(ctx, "sum_xy", 1, 4);
...
auto p = []() -> ::slinky::pipeline {
  using std::abs, std::min, std::max;
  node_context ctx;
  auto in1 = buffer_expr::make(ctx, "in1", 2, 2);
  auto in2 = buffer_expr::make(ctx, "in2", 1, 4);
  auto out1 = buffer_expr::make(ctx, "out1", 2, 2);
  auto x = var(ctx, "x");
  auto y = var(ctx, "y");
  auto intm1 = buffer_expr::make(ctx, "intm1", 2, 2);
  auto _replica_fn_2 = [=](const buffer<const void>& i0, const buffer<void>& o0) -> index_t {
    const buffer<const void>* input_buffers[] = {&i0};
    const buffer<void>* output_buffers[] = {&o0};
  ...
  auto out2 = buffer_expr::make(ctx, "out2", 1, 4);
  auto intm2 = buffer_expr::make(ctx, "intm2", 1, 4);
...
auto p = []() -> ::slinky::pipeline {
  ...
  node_context ctx;
  auto in1 = buffer_expr::make(ctx, "in1", 2, 2);
  auto in2 = buffer_expr::make(ctx, "in2", 2, 2);
  auto out = buffer_expr::make(ctx, "out", 2, 2);
  auto intm1 = buffer_expr::make(ctx, "intm1", 2, 2);
  auto x = var(ctx, "x");
  auto y = var(ctx, "y");
  auto _replica_fn_2 = [=](const buffer<const void>& i0, const buffer<void>& o0) -> index_t {
  ...
  auto intm2 = buffer_expr::make(ctx, "intm2", 2, 2);
...
auto p = []() -> ::slinky::pipeline {
  ...
  auto in1 = buffer_expr::make(ctx, "in1", 2, 2);
  auto in2 = buffer_expr::make(ctx, "in2", 2, 2);
  auto out = buffer_expr::make(ctx, "out", 3, 2);
  auto intm1 = buffer_expr::make(ctx, "intm1", 2, 2);
  auto x = var(ctx, "x");
  auto y = var(ctx, "y");
  auto _replica_fn_2 = [=](const buffer<const void>& i0, const buffer<void>& o0) -> index_t {
...
auto p = []() -> ::slinky::pipeline {
  using std::abs, std::min, std::max;
  node_context ctx;
  auto in = buffer_expr::make(ctx, "in", 2, 2);
  auto out = buffer_expr::make(ctx, "out", 2, 2);
  auto x = var(ctx, "x");
  auto y = var(ctx, "y");
  auto padded_intm = buffer_expr::make(ctx, "padded_intm", 2, 2);
  auto intm = buffer_expr::make(ctx, "intm", 2, 2);
  auto _replica_fn_3 = [=](const buffer<const void>& i0, const buffer<void>& o0) -> index_t {
    const buffer<const void>* input_buffers[] = {&i0};
    const buffer<void>* output_buffers[] = {&o0};
...
auto p = []() -> ::slinky::pipeline {
  ...
  auto x = var(ctx, "x");
  auto y = var(ctx, "y");
  auto intm3 = buffer_expr::make(ctx, "intm3", 2, 2);
  auto intm2 = buffer_expr::make(ctx, "intm2", 2, 2);
  auto _replica_fn_3 = [=](const buffer<const void>& i0, const buffer<void>& o0) -> index_t {
    const buffer<const void>* input_buffers[] = {&i0};
    const buffer<void>* output_buffers[] = {&o0};
  ...
  auto intm4 = buffer_expr::make(ctx, "intm4", 2, 2);
  auto _replica_fn_6 = [=](const buffer<const void>& i0, const buffer<void>& o0) -> index_t {
    const buffer<const void>* input_buffers[] = {&i0};
    const buffer<void>* output_buffers[] = {&o0};
