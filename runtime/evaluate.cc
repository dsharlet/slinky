...
public:
  ...
  validator(span<var> inputs, const node_context* symbols) : symbols(symbols) {
    for (var i : inputs) {
      ctx[i] = unknown;
    }
  }
  ...
  void visit_let(const T* x) {
    std::vector<set_value_in_symbol_map<variable_state>> lets;
    lets.reserve(op->lets.size());

    for (size_t i = 0; i < op->lets.size(); ++i) {
      if (!op->lets[i].defined()) {
        std::cerr << "Undefined variable ";
        print(std::cerr, x, symbols);
        std::cerr << " in context" << std::endl;
        error = true;
