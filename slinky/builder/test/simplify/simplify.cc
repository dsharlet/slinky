...
TEST(simplify, let) {
  ...
  ASSERT_THAT(simplify(let::make({{x, y * 2}, {z, x + 1}, {w, u * 2}}, w)), matches(let::make(w, u * 2, w)));
  ASSERT_THAT(simplify(let::make({{x, y * 2}, {z, x + 1}, {w, u * 2}}, z)),
      matches(let::make({{x, y * 2}, {z, x + 1}}, z)));
