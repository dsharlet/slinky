...
TEST(simplify, licm) {
  ...
  auto make_crop_x = [](var dst, var src, int dim, const stmt& body) {
    return crop_dim::make(dst, src, dim, point(x), body);
  };
  auto make_crop_y = [](var dst, var src, int dim, const stmt& body) {
    return crop_dim::make(dst, src, dim, point(y), body);
  };
  ...
  ASSERT_THAT(simplify(make_loop_x(block::make({
                  make_call(b0, b1),
                  make_crop_x(b3, b2, 0, make_call(b0, b3)),
              }))),
      matches(block::make({
          make_call(b0, b1),
          make_loop_x(make_crop_x(b3, b2, 0, make_call(b0, b3))),
      })));
  ...
  ASSERT_THAT(simplify(make_loop_x(block::make({
                  ...
                  make_call(b1, b2),
                  make_crop_x(b2, b3, 0, make_call(b0, b2)),
              }))),
  ...
  ASSERT_THAT(simplify(allocate::make(b1, memory_type::heap, 1, {{{0, 10}, 1, dim::unfolded}},
                  make_loop_x(block::make({
                      ...
                      make_call(b1, b2),
                      make_call(b1, b3),
                      make_crop_x(b1, b4, 0, make_call(b0, b1)),
                  })))),
      matches(block::make({
          ...
          make_loop_x(make_crop_x(b1, b4, 0, make_call(b0, b1))),
      })));
  ...
  // A nested loop.
  ASSERT_THAT(simplify(make_loop_y(make_crop_y(b3, b2, 1,
                  make_loop_x(block::make({
                      make_call(b0, b1),
      ...
      matches(block::make({
          make_call(b0, b1),
          make_loop_y(make_crop_y(b3, b2, 1, make_loop_x(make_crop_x(b4, b3, 0, make_call(b0, b4))))),
      })));
  ...
  ASSERT_THAT(simplify(make_loop_x(block::make({
                  ...
                  make_call(b1, b2),
                  make_call(b2, b3),
                  make_crop_x(b0, b4, 0, make_call(b3, b0)),
              }))),
      matches(make_loop_x(block::make({
          make_crop_x(b2, b1, 0, make_call(b0, b2)),
          make_call(b1, b2),
...
TEST(simplify, clone) {
  ...
  ASSERT_THAT(simplify(crop_dim::make(x, u, 1, point(10),
      ...
      matches(crop_dim::make(x, u, 1, point(10),
          make_buffer::make(
              z, buffer_at(w), buffer_elem_size(w), {buffer_dim(x, 0), buffer_dim(x, 1)}, dummy_call({}, {x, z})))));
}
...
TEST(simplify, slice_of_const_buffer) {
...

  {
    ...
    ASSERT_THAT(simplify(let_stmt::make(b1, constant_buffer::make(constant_buf), slice_dim::make(b3, b1, 1, 0, body))),
        matches(let_stmt::make(b3, constant_buffer::make(sliced_constant_buf), body)));
  }

  {
    ...
    ASSERT_THAT(simplify(let_stmt::make(b1, constant_buffer::make(constant_buf), slice_buffer::make(b3, b1, {5, 10}, body))),
        matches(let_stmt::make(b3, constant_buffer::make(sliced_buf), body)));
  }

  {
    ...
    ASSERT_THAT(simplify(let_stmt::make(b1, constant_buffer::make(constant_buf), slice_buffer::make(b3, b1, {5, 21}, body))),
        matches(let_stmt::make(b3, constant_buffer::make(sliced_buf), body)));
  }
  {
    ...
    for (int at = 0; at <= 1; ++at) {
      for (int d = 0; d < 3; ++d) {
        ...
        ASSERT_THAT(simplify(let_stmt::make(b1, constant_buffer::make(constant_buf), slice_dim::make(b3, b1, d, at, body))),
            matches(let_stmt::make(b3, constant_buffer::make(sliced_buf), body)));
      }
    }
    ...
    for (int at = 0; at <= 1; ++at) {
      for (int d = 0; d < 3; ++d) {
        ...
        ASSERT_THAT(simplify(let_stmt::make(b1, constant_buffer::make(constant_buf), slice_buffer::make(b3, b1, ats, body))),
            matches(let_stmt::make(b3, constant_buffer::make(sliced_buf), body)));
      }
    }
...

  {
    ...
    ASSERT_THAT(simplify(let_stmt::make(b1, constant_buffer::make(constant_buf),
        ...
        matches(let_stmt::make({{b1, constant_buffer::make(constant_buf)}, {b4, constant_buffer::make(sliced_buf)}},
            dummy_call({b4, b1}, {}))));
  }
}
