  ...
  void visit(const transpose* op) override {
    ...
    if (src_info && *src_info) {
      ...
      for (size_t i = 0; i < dims.size(); ++i) {
        if (0 <= dims[i] && dims[i] < static_cast<int>((*src_info)->dims.size())) {
...
class constant_bounds_evaluator {
  // The input expressions are often heavily shared DAGs: interval arithmetic (e.g. `bounds_of`)
  // reuses subexpression bounds many times. Memoize per node so the walk is linear in the number
  // of unique nodes instead of exponential in the depth of the sharing. Raw pointer keys are safe
  // here because this evaluator creates no temporary expressions: every visited node is reachable
  // from the root, which the caller keeps alive.
