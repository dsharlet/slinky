...
// A `variable` packs into the inline payload of a `tagged_node_ptr` as
// [ sym.id : 32 | dim : 7 | field : 3 ], 42 bits of the 62 the payload holds.
// `dim` gets 7 bits because `max_rank` is an inclusive bound, so it must
// represent 0..64.
constexpr int variable_field_bits = 3;
constexpr int variable_dim_bits = 7;
static_assert(max_rank < (1 << variable_dim_bits), "variable::dim does not fit in the inline payload");
static_assert(32 + variable_dim_bits + variable_field_bits <= 8 * sizeof(std::uintptr_t) - tagged_node_ptr::tag_bits,
    "variable does not fit in the inline payload");

SLINKY_INLINE std::uintptr_t pack_variable(var sym, buffer_field field, int dim) {
  assert(static_cast<unsigned>(field) < (1 << variable_field_bits));
  assert(dim >= 0 && dim <= max_rank);
  return (static_cast<std::uintptr_t>(sym.id) << (variable_dim_bits + variable_field_bits)) |
         (static_cast<std::uintptr_t>(dim) << variable_field_bits) | static_cast<std::uintptr_t>(field);
}
SLINKY_INLINE variable unpack_variable(std::uintptr_t p) {
  variable v;
  v.field = static_cast<buffer_field>(p & ((1 << variable_field_bits) - 1));
  v.dim = static_cast<int>((p >> variable_field_bits) & ((1 << variable_dim_bits) - 1));
  v.sym = var(static_cast<var::type>(p >> (variable_dim_bits + variable_field_bits)));
  return v;
}
